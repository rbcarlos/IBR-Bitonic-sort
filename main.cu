...
)
{
    ...
    BS_firstStagesKernel<<<numBlocks, numThreads, sharedMemSize>>>(d_keys);
    ...
    for (int stage = stagesBitonicSort + 1; stage <= stagesAll; stage++)
    {
        ...
        // BS_2_IBR + IBR_stages
        IBRKernel<typename OpTp::ElTp><<<numBlocks, numThreads, sharedMemSize>>>(d_keys, d_intervals, arrayLength, stepStart, stepEnd);
        ...
        while (stepEnd > stagesInMemory)
        {
            ...
            // only IBR_stages
            IBRContKernel<<<numBlocks, numThreads, sharedMemSize>>>(d_keys, d_intervalsBuffer, d_intervals, arrayLength, stage, stepStart, stepEnd);
        }
